proxyz <- function(X, Y, tau = 0.5, lambda, weights = NULL,
                   constraint = c("none", "increase", "decrease"),
                   PeltyD = NULL, pord = 2, delta = 1e-6, iter_out = 1e6, 
    ...
    #           constrainted quantile regression coefficients estimates.
    #
    # Refer:
    ...
    beta_est <- proxy_inital(Y, X, tau, weights, model, delta, lambda, PeltyD, iter_out, iter_in, threshold)
